...
from scholarai.api.rag_service import course_code, course_color, run_ask
from scholarai.api.schemas import CourseCreate, CourseOut, CourseStats, ArtifactItem, JobOut, PackageMeta
...
def create_course(payload: CourseCreate) -> CourseOut:
    ...
    session = get_session()
    try:
        from scholarai.storage.models import get_course, Course
...
def get_course_stats(course_id: int) -> CourseStats:
    ...
    try:
        ...
        try:
            from scholarai.storage.models import DepConcept
...
def get_course_artifacts(
    ...
    try:
        ...
        if type is None or type == "learning_path":
            from scholarai.storage.models import LearningPath
...
async def generate_course_package(course_id: int) -> PackageMeta:
    ...
    try:
        ...
        session.commit()
        session.refresh(pkg)
        from scholarai.api.schemas import PackageMeta as _PackageMeta
