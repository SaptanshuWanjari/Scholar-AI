...
def test_cache_miss_returns_none(db):
    from scholarcli.ingest.page_cache import get_cached_ocr
    assert get_cached_ocr("nonexistent_hash") is None


def test_store_then_get_returns_text(db):
    from scholarcli.ingest.page_cache import store_ocr, get_cached_ocr
...
def test_image_hash_is_deterministic():
    from scholarcli.ingest.page_cache import image_hash
