...
const SIZES = {
  sm: "min-h-14 py-2 px-4 text-[16px]",
  md: "min-h-15 py-3 px-6 text-[18px]",
  lg: "min-h-17 py-3.5 px-8 text-[20px]",
};

/** Base hand-drawn button. Press lifts 1px up, active settles down. */
export const PaperButton = React.forwardRef<
  HTMLButtonElement,
  PaperButtonProps
>(function PaperButton(
  { children, className, tone = "paper", size = "md", border, style, ...props },
  ref,
) {
  const t = TONES[tone];
  return (
...
>(function StickyButton(
  { children, taped = true, tone = "paper", className, ...props },
  ref,
) {
  return (
...
const FAB_SIZES: Record<"sm" | "md" | "lg", number> = {
  sm: 40,
  md: 52,
  lg: 64,
};
...
export const ChipButton = React.forwardRef<HTMLButtonElement, ChipButtonProps>(
  function ChipButton(
    { children, selected = false, tone, border, className, style, ...props },
    ref,
  ) {
    ...
    return (
      <button
        ...
        className={cn(
          "relative inline-flex h-7 items-center gap-1.5 px-3 py-2 font-architect text-[13px]",
...
>(function GhostButton(
  { children, size = "md", border, className, style, ...props },
  ref,
) {
  return (
    <button
