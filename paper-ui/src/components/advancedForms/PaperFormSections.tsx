...
export interface PaperFormSectionProps extends React.HTMLAttributes<HTMLFieldSetElement> {
  title?: string;
  description?: string;
}

export const PaperFormSection = React.forwardRef<HTMLFieldSetElement, PaperFormSectionProps>(
  function PaperFormSection({ title, description, children, className, ...props }, ref) {
    return (
      <fieldset ref={ref} className={cn("mb-8 w-full", className)} {...props}>
        {title && (
          <legend className="font-architect font-bold text-[20px] text-ink mb-1 px-1 relative inline-block">
            {title}
