/**
 * Inline SVG icons for driver.js popover titles. driver.js renders the title via
 * innerHTML, so we hand it a small HTML string with a lucide SVG + the text.
 * Paths are copied verbatim from lucide-react (v0.487) so they match the app.
 */
...
/** Build a popover title with a leading icon. */
export function tourTitle(icon: string, text: string): string {
  return `<span class="tour-title">${icon}<span>${text}</span></span>`;
}
