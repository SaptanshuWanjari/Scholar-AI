...
import {
  PaperInput,
  PaperTextarea,
} from "@/paper-ui/components/inputs/PaperInput";
...
function Accuracy({ value }: { value: number | null }) {
  if (value == null)
    return (
      <span className="text-xs text-muted-foreground/60">Not practiced</span>
    );
  const tone =
    value >= 75 ? "text-success" : value >= 50 ? "text-warning" : "text-danger";
...
export function PyqAnalysis() {
  ...
  const seedExam = () => {
    if (!analysis) return;
    const top = analysis.topicFrequency
      .slice(0, 5)
      .map((t) => t.topic)
      .join(", ");
    ...
    useExamStore
      .getState()
      .setField("difficulty", analysis.summary.avgDifficulty || "Medium");
    useExamStore.getState().setField("types", ["mcq", "short"]);
    toast.message("Generating Mock Exam", {
      description: "Configured from PYQ trends.",
    });
    navigate("/exam");
    setTimeout(() => {
  ...
  const genFlashcards = (topic: string) => {
    navigate(
      `/flashcards?topic=${encodeURIComponent(topic)}&course=${encodeURIComponent(course)}`,
    );
  };
  ...
  const handleUploadFile = async (file: File) => {
    ...
    toast.success(
      `Extracted ${res.extracted} questions from ${res.paper.title}`,
    );
  };
      ...
      <Container className="max-w-6xl py-6 font-kalam text-ink">
          ...
          <div>
            {hasData && (
              <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground">
                <span>
                  Analyzed: <b>{analysis!.papers}</b> papers
                </span>
                {analysis!.yearsLabel && (
                  <span>
                    Years: <b>{analysis!.yearsLabel}</b>
                  </span>
                )}
                <span>
                  Questions: <b>{analysis!.totalQuestions}</b>
                </span>
              </div>
            )}
          </div>
        ...
        {!course && (
          <p className="text-sm text-muted-foreground">
            Create a course first to analyze its papers.
          </p>
        )}
...
function SummaryCards({
  ...
  a: NonNullable<ReturnType<typeof usePyqStore.getState>["analysis"]>;
}) {
  const s = a.summary;
  const cards = [
    {
      label: "Topics Identified",
      value: s.topicsIdentified ?? 0,
      icon: Layers,
      color: "text-violet",
    },
    {
      label: "Recurring Topics",
      value: s.recurringTopics ?? 0,
      icon: RefreshCw,
      color: "text-cyan",
    },
    { label: "Question Types", value: s.questionTypes ?? 0, icon: ListChecks, color: "text-warning" },
    {
      label: "Avg Difficulty",
      value: s.avgDifficulty ?? "—",
      icon: AlertTriangle,
      color: "text-danger",
    },
    { label: "Coverage", value: `${s.coverage ?? 0}%`, icon: BookOpen, color: "text-success" },
    {
    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {cards.map((c) => (
        <PaperMetricCard
          ...
          label={c.label}
          value={c.value}
          icon={<c.icon className={`size-5 ${c.color}`} />}
        />
      ))}
    </div>
  );
...
function TopicFrequency({
  ...
  return (
    <PaperCard className="rounded-2xl border border-border bg-card p-5">
      <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-violet/80">
        Topic Frequency
      </h3>
      <div className="max-h-[420px] overflow-y-auto">
        <table className="w-full text-sm">
          <thead className="text-left text-xs text-violet/70">
          ...
          <tbody>
            {rows.map((r) => (
              ...
              >
                <td className="py-2">
                  <button
                    className="text-left font-medium hover:text-violet"
                    onClick={() => onOpen(r.topic)}
                  >
                    {r.topic}
                  </button>
                </td>
    ...
    <PaperCard className="rounded-2xl border border-border bg-card p-5">
      ...
      <div className="grid grid-cols-2 gap-2">
        {patterns.map((p) => (
          <PaperButton
            key={p.type}
            className={selected === p.type ? "border-violet/30 bg-violet/5" : ""}
            onClick={() =>
              setField("selectedPattern", selected === p.type ? null : p.type)
            }
          >
            <div className={`font-display font-kalam text-xl ${selected === p.type ? "text-violet" : ""}`}>{p.pct}%</div>
            <div className={`text-xs ${selected === p.type ? "text-violet/70" : "text-muted-foreground"}`}>{p.label}</div>
          </PaperButton>
        ))}
      </div>
      {active && (
...
function YearTimeline({
  ...
  trends: { topic: string; years: { year: number; count: number }[] }[];
}) {
...
function DifficultyDistribution({
  ...
  dist: { level: string; count: number }[];
}) {
        ...
        {dist.map((d) => (
          <div key={d.level}>
            ...
            <div className="h-2 overflow-hidden rounded-full bg-muted">
              <div
                className={`h-full ${tone[d.level] ?? "bg-violet"}`}
...
function MarksDistribution({
  ...
  dist: { marks: number; count: number }[];
}) {
  const total = Math.max(
    1,
    dist.reduce((a, d) => a + d.count, 0),
...
function DifferenceSuggestions() {
  ...
  return (
    <PaperCard className="rounded-2xl border border-border bg-card p-5">
      <h3 className="mb-1 text-sm font-semibold uppercase tracking-wider text-violet/80">
      ...
      {differences.length === 0 ? (
        <p className="text-sm text-muted-foreground">
          No comparison questions detected yet.
        </p>
      ) : (
        <div className="space-y-2">
          {differences.map((d, i) => (
            <button
              ...
              onClick={() =>
                navigate(
                  `/differences?a=${encodeURIComponent(d.a)}&b=${encodeURIComponent(d.b)}`,
                )
              }
            ...
            >
              <span className="text-sm">
                <b>{d.a}</b> <span className="text-muted-foreground">vs</span>{" "}
                <b>{d.b}</b>
              </span>
              <span className="flex items-center gap-2 text-xs text-muted-foreground">
                {d.count > 1 && (
                  <PaperBadge className="text-xs">{d.count}×</PaperBadge>
                )}
                <Columns2 className="size-4 text-violet/60" />
              </span>
            </button>
          ))}
        </div>
...
function RevisionRisk({
  ...
  return (
    <PaperCard className="rounded-2xl border border-border bg-card p-5">
      ...
      <p className="mb-3 text-xs text-muted-foreground">
        High frequency + low accuracy = high risk. Based on topics you've
        practiced.
      </p>
      {risk.length === 0 ? (
        <p className="text-sm text-muted-foreground">
          Take a PYQ mock exam to build per-topic accuracy and surface revision
          priorities.{" "}
          <button className="text-violet hover:underline" onClick={onExam}>
            Generate one →
          </button>
        </p>
      ) : (
        <div className="space-y-2">
          {risk.map((r) => (
            <div
              key={r.topic}
              className={`flex flex-wrap items-center justify-between gap-3 rounded-lg border p-3 ${tone[r.risk]}`}
            >
              <div>
    ...
    <PaperCard className="rounded-2xl border border-border bg-card p-5">
      ...
      {(weak.length > 0 || strong.length > 0) && (
        <div className="mt-4 flex flex-wrap gap-2">
          {strong.map((t) => (
            <PaperBadge key={t} className="border-success/40 text-success">
              {t}
            </PaperBadge>
          ))}
          {weak.map((t) => (
            <PaperBadge key={t} className="border-danger/40 text-danger">
              {t}
            </PaperBadge>
          ))}
        </div>
      ...
      {weak.length === 0 && strong.length === 0 && (
        <p className="mt-3 text-xs text-muted-foreground">
          No practice data yet — readiness is an estimate built from PYQ
          mock-exam results.
        </p>
      )}
    </PaperCard>
...
function QuestionCard({ q }: { q: PyqQuestion }) {
  ...
  if (editing) {
    return (
      <PaperPanel className="rounded-lg border border-border bg-muted/30 p-3 space-y-2">
        <textarea
  ...
  return (
    <PaperPanel className="group rounded-lg border border-border p-3">
      ...
      <div className="mt-2 flex flex-wrap gap-1.5 text-xs">
        ...
        {q.subtopics?.map((st) => (
          <PaperBadge key={st} className="text-violet">
            {st}
          </PaperBadge>
        ))}
...
function PaperList({
  ...
  papers: ReturnType<typeof usePyqStore.getState>["papers"];
}) {
    ...
    <div className="flex items-center justify-between p-3">
      <div className="flex items-center gap-2">
        ...
        <div className="min-w-0">
          ...
          <div className="text-xs text-muted-foreground">
            {p.year ? `${p.year} · ` : ""}
            {p.questionCount} questions · {p.createdAt}
          </div>
        </div>
      </div>
...
function TopicDrawer({
  ...
  return (
    <PaperDrawer
      open={!!topic}
      onClose={() => setField("selectedTopic", null)}
        ...
        <div className="space-y-4 text-sm">
          ...
          {row.subtopics.length > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {row.subtopics.map((s) => (
                  <PaperBadge key={s} className="text-violet">
                    {s}
                  </PaperBadge>
                ))}
              </div>
          ...
          {row.styles.length > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {row.styles.map((s) => (
                  <PaperBadge key={s}>{s.replace(/_/g, " ")}</PaperBadge>
                ))}
              </div>
            </div>
          )}
          {trend && <YearTimeline trends={[trend]} />}
          <div className="flex flex-wrap gap-2 pt-1">
            <GhostButton size="sm" onClick={() => onNotes(row.topic)}>
              <NotebookPen className="size-4" /> Notes
            </GhostButton>
            <GhostButton size="sm" onClick={() => onFlashcards(row.topic)}>
              <Layers className="size-4" /> Flashcards
            </GhostButton>
            <GhostButton size="sm" onClick={() => onQuiz(row.topic)}>
              <ListChecks className="size-4" /> Quiz
            </GhostButton>
            <PaperButton size="sm" onClick={onExam}>
              <GraduationCap className="size-4" /> Mock Exam
            </PaperButton>
          </div>
        </div>
      )}
