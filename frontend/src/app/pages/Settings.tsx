  ...
  return (
    <div className="flex items-center justify-between gap-6 border-b border-[#e8e3d8] py-4 last:border-0">
      <div className="min-w-0">
        <div className="font-architect text-[19px] text-ink">{title}</div>
        {desc && (
          <div className="font-kalam text-[16px] text-ink-muted/75">{desc}</div>
        )}
      </div>
...
export function SettingsPage() {
  ...
  const [modelsSubTab, setModelsSubTab] = useState<
    "providers" | "routing" | "defaults" | "usage"
  >("providers");
  ...
  const [backups, setBackups] = useState<
    Array<{ path: string; stamp: string; size_mb: number }>
  >([]);
  ...
  const handleReindexAll = async () => {
    ...
    try {
      ...
      setBackupMsg(
        `Rebuilding all documents: job queued (${job.id.slice(0, 8)}…).`,
      );
    } catch (e: any) {
  ...
  const handleBackup = async () => {
    ...
    try {
      ...
      setBackupMsg(
        `Backup created: ${res.backup.stamp} (${res.backup.size_mb} MB)`,
      );
    ...
    <Page className="max-w-5xl">
      <PaperCard className="w-full" shadow="md">
        <div className="flex flex-col md:flex-row">
          ...
          <div
            className="w-full md:w-64 shrink-0 p-4"
          ...
          <div className="flex-1 w-full min-w-0 p-6 md:p-8 space-y-8">
            {activeTab === "appearance" && (
              <div>
                <Row
                  title="Font Size"
                  desc={`Base font size: ${appearance.fontSize}px`}
                >
                  <PaperSlider
                    className="w-56"
                ...
                <Row
                  title="Reading Font"
                  desc="Font used for documents and long text"
                >
                  <PaperSelect
              ...
              <div className="p-4">
                <PaperCard className="p-5 flex flex-col items-center text-center gap-3">
                  <div className="flex justify-center w-full">
                    <Cloud size={33} className="text-ink-muted" />
                  </div>
                  <div>
                    <div className="font-architect text-lg text-ink">
                      Enable Cloud Providers
                    </div>
                    <p className="font-kalam text-sm text-ink-muted mt-1 max-w-xs">
                      Install the plugin to connect Gemini, Groq, OpenRouter, or
                      any OpenAI-compatible endpoint and configure model
                      routing.
                    </p>
                  </div>
                  <PaperButton
                  ...
                  >
                    {getInstallState("cloud-model-providers") ===
                      "installing" ? (
                      <>
            ...
            {activeTab === "models" &&
              isEnabled("cloud-model-providers") &&
              modelsSubTab === "providers" && <ModelProvidersTab />}
            {activeTab === "models" &&
              isEnabled("cloud-model-providers") &&
              modelsSubTab === "routing" && <ModelRoutingTab />}
            {activeTab === "models" &&
              isEnabled("cloud-model-providers") &&
              modelsSubTab === "usage" && <ModelUsageTab />}

            {activeTab === "models" &&
              isEnabled("cloud-model-providers") &&
              modelsSubTab === "defaults" && (
                <div className="">
                  <ModelDefaultsTab />
                  <Row
                    title="AI Mode"
                  ...
                  <Row
                    ...
                    desc="Render tokens as they generate"
                  >
                    <PaperSwitch
                      checked={s.streaming}
                  ...
                  <Row
                    ...
                    desc="Automatically refine and improve your prompts"
                  >
                    <PaperSwitch
            ...
            {activeTab === "models" &&
              (!isEnabled("cloud-model-providers") ||
                ...
                <>
                  <div>
                    <h3 className="pt-4 font-architect text-[18px] text-ink-muted">
                      Embedding &amp; Vision
                    </h3>
                    <Row
                    ...
                    <Row title="Vector store" desc="Local LanceDB storage">
                      <span
                        className="rounded-md px-3 py-1.5 font-mono text-xs text-ink-muted"
                    ...
                    >
                      <div className="flex flex-col items-end gap-1.5">
                        ...
                        <div className="flex items-center gap-2 text-xs">
                          <span
                            className={`font-kalam rounded px-1.5 py-0.5 font-medium ${s.maxConcurrent <= 2
                          ...
                          <span className="font-kalam text-muted-foreground">
                            {s.maxConcurrent <= 2
            ...
            {activeTab === "profile" && (
              <div>
                ...
                <Row
                  ...
                  desc="How you prefer to learn"
                >
                  <PaperInput
                    className="w-56"
                    value={s.learningPreferences}
                    onChange={(e) =>
                      s.set("learningPreferences", e.target.value)
                    }
                        ...
                        <PluginRow
                          ...
                          control={
                            <PaperSwitch
                              checked={enabled}
                              onChange={() =>
                                enabled ? disable(plugin.id) : enable(plugin.id)
                              }
            ...
            {activeTab === "data" && (
              <>
                <div>
                  ...
                  {backupMsg && (
                    <div className="pb-4 font-kalam text-xs text-ink-muted/75">
                      {backupMsg}
                    </div>
                  )}
                  <Row
                  ...
                  {backups.length > 0 && (
                    <div className="pb-4 space-y-1">
                      <div className="font-architect text-xs text-ink-muted">
                        Recent backups
                      </div>
                      {backups.map((b) => (
                        <div
                          key={b.stamp}
                          className="font-kalam text-xs text-ink-muted/75 flex justify-between"
                        >
                          <span>{b.stamp}</span>
