...
function InputPhase() {
  const { topic, depth, selected, setField, toggleArtifact, startGenerate, loadPackage } =
    useTeachStore();
        ...
        return (
          <div key={q.id} className="rounded-xl border border-border bg-card p-4">
            ...
            <div className="mt-3 pl-7">
              {show ? (
                <div className="rounded-lg border border-success/30 bg-success-soft/50 p-3 text-sm">
                  <div className="flex items-center gap-1.5 font-medium text-success">
                    <CheckCircle2 className="size-3.5" /> {q.answer}
                  </div>
...
function ArtifactBody({ view }: { view: ArtifactKey }) {
  ...
  switch (view) {
    ...
    case "diagram":
      return (
        <div className="h-[60vh] overflow-auto rounded-xl border border-border bg-card p-4">
          <DiagramViewer code={(slot.data as GeneratedDiagram).mermaid} />
        </div>
      );
    case "mindmap": {
